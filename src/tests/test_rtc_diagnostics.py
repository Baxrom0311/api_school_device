...
class TestRTCDriftAlertHandler:
    ...
    def test_rtc_drift_low_battery(self, mqtt_listener, device):
        mqtt_listener._handle_alert(device.device_id, {"type": "rtc_drift", "drift_sec": 312, "battery_status": "low"})
    ...
    def test_rtc_drift_dead_battery(self, mqtt_listener, device):
        mqtt_listener._handle_alert(device.device_id, {"type": "rtc_drift", "drift_sec": 900, "battery_status": "dead"})
    ...
    def test_rtc_drift_idempotent(self, mqtt_listener, device):
        ...
        mqtt_listener._handle_alert(device.device_id, {"type": "rtc_drift", "drift_sec": 300, "battery_status": "low"})
        mqtt_listener._handle_alert(device.device_id, {"type": "rtc_drift", "drift_sec": 350, "battery_status": "low"})
...
class TestHeartbeatRTCReset:
    ...
    def test_rtc_ok_resets_battery_status(self, mqtt_listener, device):
        ...
        Device.objects.filter(id=device.id).update(rtc_synced=False, rtc_battery_status="low", rtc_drift_sec=300)
    ...
    def test_rtc_ok_resolves_alerts(self, mqtt_listener, device):
        ...
        assert not DeviceAlert.objects.filter(
            device=device, resolved=False, alert_type__in=["rtc_drift", "rtc_battery_dead"]
        ).exists()
