...
class Schedule(AbstractBaseModel):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk:
            old = (
                Schedule.objects.filter(pk=self.pk)
                .values("times", "days_mask", "bell_duration", "version", "device_id")
                .first()
            )
            if old and (
                old["times"] != self.times
                or old["days_mask"] != self.days_mask
                or old["bell_duration"] != self.bell_duration
            ):
