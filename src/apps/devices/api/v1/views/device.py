...
class DeviceViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_authenticated and getattr(self.request.user, "role", None) not in (
            "ADMIN",
            "SUPERADMIN",
        ):
    ...
    def stats(self, request):
        ...
        firmware_counts = queryset.values("firmware_version").annotate(count=Count("id")).order_by("-count")
        firmware_versions = {item["firmware_version"]: item["count"] for item in firmware_counts}
    ...
    def regenerate_credentials(self, request, pk=None):
        ...
        serializer = self.get_serializer(device)
        return Response({**serializer.data, "warning": "Device must be reflashed with new credentials!"})
    ...
    def regenerate_api_key(self, request, pk=None):
        ...
        serializer = self.get_serializer(device)
        return Response({**serializer.data, "warning": "Device must be reflashed with new API key!"})
    ...
    def unregistered(self, request):
        ...
        """
        queryset = self.get_queryset().filter(registration_status=RegistrationStatus.UNREGISTERED)
    ...
    def pending(self, request):
        ...
        """
        queryset = self.get_queryset().filter(registration_status=RegistrationStatus.PENDING).order_by("-created_at")
