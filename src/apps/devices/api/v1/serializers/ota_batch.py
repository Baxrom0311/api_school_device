...
class OTABatchDeviceSerializer(serializers.ModelSerializer):
    ...
    current_version = serializers.CharField(source="device.firmware_version", read_only=True)
...
class OTABatchCreateSerializer(serializers.Serializer):
    ...
    def validate_firmware_id(self, value):
        ...
        except FirmwareVersion.DoesNotExist as exc:
            raise serializers.ValidationError(_("Firmware version not found")) from exc

        if not firmware.is_stable:
            raise serializers.ValidationError(_("Cannot create batch with unstable firmware. Mark as stable first."))
    ...
    def validate_device_ids(self, value):
        ...
        pending_devices = OTABatchDevice.objects.filter(
            device_id__in=value, batch__status__in=[OTABatchStatus.PENDING, OTABatchStatus.IN_PROGRESS]
        ).values_list("device_id", flat=True)

        if pending_devices:
            raise serializers.ValidationError(f"Devices already in pending OTA: {list(pending_devices)}")
    ...
    def validate(self, attrs):
        ...
        if incompatible:
            raise serializers.ValidationError(
                {
                    "device_ids": _(f"{len(incompatible)} devices incompatible with firmware."),
