...
class ScheduleUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_times(self, value):
        ...
        if len(value) > 100:
            raise serializers.ValidationError(_("Maximum 100 times allowed per schedule"))
        ...
        for i, time in enumerate(value):
            ...
            if not TIME_PATTERN.match(time):
                errors.append(f"Invalid time format at index {i}: '{time}'. Use HH:MM (24h)")
                continue
    ...
    def update(self, instance, validated_data):
        ...
        sync_needed = (
            ("times" in validated_data and validated_data["times"] != instance.times)
            or ("days_mask" in validated_data and validated_data["days_mask"] != instance.days_mask)
            or ("bell_duration" in validated_data and validated_data["bell_duration"] != instance.bell_duration)
        )
    ...
    def validate_device(self, value):
        ...
        if request and getattr(request.user, "role", None) not in ("ADMIN", "SCHOOL_ADMIN"):
            if value.owner != request.user:
                raise serializers.ValidationError(_("You can only create schedules for your own devices"))
        if Schedule.objects.filter(device=value).exists():
            raise serializers.ValidationError(_("This device already has a schedule"))
        return value
