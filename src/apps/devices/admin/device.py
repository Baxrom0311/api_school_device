...
class DeviceAdmin(ModelAdmin):
    ...
    def display_registration_status(self, obj):
        if obj.registration_status == "registered":
            return format_html('<span style="color: #10b981;">● Registered</span>')
        elif obj.registration_status == "pending":
            return format_html('<span style="color: #f59e0b;">● Pending</span>')
        else:
            return format_html('<span style="color: #6b7280;">● Unregistered</span>')
    ...
    def display_rtc_status(self, obj):
        if obj.rtc_synced:
            ...
            return format_html('<span style="color: #ef4444;">✗ Error</span>')
    ...
    def ring_devices(self, request, queryset):
        ...
        self.message_user(request, f"Ring command sent to {success_count}/{queryset.count()} devices")
    ...
    def restart_devices(self, request, queryset):
        ...
        self.message_user(request, f"Restart command sent to {success_count}/{queryset.count()} devices")
    ...
    def sync_schedules(self, request, queryset):
        ...
        self.message_user(request, f"Schedules synced to {success_count}/{queryset.count()} devices")
    ...
    def approve_registration(self, request, queryset):
        ...
        count = queryset.filter(registration_status="pending").update(
            registration_status="registered", registered_at=timezone.now()
        )
